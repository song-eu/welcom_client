...
const ChangeGeoLocation = async (geojson, dataloc, currDate) => {
    //console.log(mapData)
    var lastMonth = moment().subtract(2, 'month').format('YYYY-MM') // 2021-07 or 마지막 달
    var result = {}
    ...
    return d3
        .csv(process.env.PUBLIC_URL + dataloc, (map) => {
            // 데이터 월 = 마지막 달
            if (map.DATE.substring(0, 7) === currDate) {
                if (map.SIDO_ADDR != '') {
                            ...
                            if (!result[map.SIDO_ADDR.substring(0, 2)]) {
                                if (
                                    ...
                                    result[map.SIDO_ADDR.substring(0, 2)] = {
                                        id: geojson.features[i].properties.code,
                                        name: geojson.features[i].properties
                                            .name,
                                        cnt: 0,
        ...
        .then(() => {
            ...
            console.log('map infom', resultArr)
            return resultArr
        })
}
